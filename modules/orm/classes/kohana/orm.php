...
class Kohana_ORM extends Model implements serializable {
    ...
    public function add_main($alias, $far_keys)
    {
        $far_keys = ($far_keys instanceof ORM) ? $far_keys->pk() : $far_keys;

        $columns = array($this->_has_many[$alias]['far_key'], $this->_has_many[$alias]['foreign_key']);
        $foreign_key = $this->pk();
	 ...
	 * @return ORM
	 */
    public function remove_main($alias, $far_keys = NULL)
    {
        $far_keys = ($far_keys instanceof ORM) ? $far_keys->pk() : $far_keys;

        $query = DB::delete($this->_has_many[$alias]['through'])
            ->where($this->_has_many[$alias]['far_key'], '=', $this->pk());

        if ($far_keys !== NULL)
        {
            // Remove all the relationships in the array
            $query->where($this->_has_many[$alias]['foreign_key'], 'IN', (array) $far_keys);
        }
